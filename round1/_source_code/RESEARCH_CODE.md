...
> **Assets:** INTARIAN_PEPPER_ROOT Â· ASH_COATED_OSMIUM  
...
# Compute fair value deviation: Fair = 10,000 + 0.001Â·t
...
FV_t = Î±Â·t + Î²
```
where Î± (slope) and Î² (intercept) are estimated via linear regression on historical mid-prices.
...
![png](_analysis_img/round1_analysis_21_0.png)
    ...
    [!] ê²°ê³¼: ?¼ì°¨ë³??‰ê·  ?¤í”„?ˆë“œê°€ ê¾¸ì???'ì¦ê?'?˜ê³  ?ˆìŠµ?ˆë‹¤.
...
def plot_pepper_mid_l3_robust(day=0, num_intervals=30):
    ...
    try:
        ...
        if pepper_l3.empty:
            print(f"? ï¸ Warning: {filename} has no Level 3 quote data for PEPPER!")
...
```
    ...
    ? ï¸ ê²½ê³ : prices_round_1_day_0.csv ?Œì¼??Pepper??Level 3 ?¸ê? ?°ì´?°ê? ?˜ë‚˜???†ìŠµ?ˆë‹¤!
    ?€??Level 1 ì¤‘ê°„ê°€(Mid Price)ë¥??•ì¸?©ë‹ˆ??
...
def plot_pepper_mid_l2_30_intervals(day=0, num_intervals=30):
    ...
    try:
        ...
        if pepper_l2.empty:
            print(f"? ï¸ {filename}has insufficient Level 2 data. Falling back to Level 1 mid price.")
...
def plot_asset_3day_detailed(product_name='ASH_COATED_OSMIUM', num_intervals_per_day=10):
    ...
    # Create 30 subplots (3 days Ã— 10 intervals)
