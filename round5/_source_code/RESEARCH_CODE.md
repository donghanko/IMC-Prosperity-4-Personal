...
> ?†Ô∏è **This notebook is intentionally unpolished.** It documents a live search for a lead-lag signal that ultimately yielded no robust out-of-sample result. The analytical post-mortem ??what went wrong and why ??is in [`round5.md`](../round5.md).
...
```python
# ?Ä?Ä Product-level quantitative summary ?Ä?Ä
if 'prices' in locals():
...
```python
# ?Ä?Ä Group-level summary (sorted by spread) ?Ä?Ä
if 'prices' in locals():
    ...
    for cat in sorted(CATEGORIES.keys()):
        ...
        if pairs:
            ...
            for p1, p2, c in pairs:
                bar = '?? * int(abs(c) * 20)
                color = '?ü¢' if c > 0.1 else ('?î¥' if c < -0.1 else '??)
...
```python
if 'prices' in locals():
    # ?Ä?Ä OBI ??next-tick return predictive power ?Ä?Ä
    print('='*80)
...
PCA-derived minimum-variance basket across all 5 SNACKPACK variants. Weights: CHOC √ó1.0, VANI √ó0.958, PISTA √ó??.111, STRAW √ó0.138, RASP √ó??.016.
...
> ?†Ô∏è **Critical flaw:** All tests below were run on **Day 2 only**. No OOS validation across Day 3 or Day 4 was completed before time ran out. This is the core failure ??signals that appeared directional on Day 2 were not verified to be stable.
...
```
    ...
    === TRANSLATOR_GRAPHITE_MIST Í∞ÄÍ≤?Î≥Ä?îÎüâ (t -> t+1) ?µÍ≥Ñ ===
    
    [TRANSLATOR_SPACE_GRAY]
     - OBI >= 0.5  ?âÍ∑† Í∞ÄÍ≤?Î≥Ä?? +0.3179 (?úÎ≥∏ ?? 173)
     - OBI <= -0.5 ?âÍ∑† Í∞ÄÍ≤?Î≥Ä?? -0.3533 (?úÎ≥∏ ?? 184)
    
    [TRANSLATOR_ECLIPSE_CHARCOAL]
     - OBI >= 0.5  ?âÍ∑† Í∞ÄÍ≤?Î≥Ä?? +0.2315 (?úÎ≥∏ ?? 203)
     - OBI <= -0.5 ?âÍ∑† Í∞ÄÍ≤?Î≥Ä?? -0.3333 (?úÎ≥∏ ?? 222)
    
    [TRANSLATOR_VOID_BLUE]
     - OBI >= 0.5  ?âÍ∑† Í∞ÄÍ≤?Î≥Ä?? +1.0920 (?úÎ≥∏ ?? 87)
     - OBI <= -0.5 ?âÍ∑† Í∞ÄÍ≤?Î≥Ä?? -0.1893 (?úÎ≥∏ ?? 103)
...
```
    ...
    === TRANSLATOR_GRAPHITE_MIST ÏßÑÏûÖ ??N???ÑÏ†Å Í∞ÄÍ≤?Î≥Ä???òÏùµ) ?âÍ∑† ===
    
    [TRANSLATOR_SPACE_GRAY]
     OBI >= 0.5 (Îß§Ïàò) 1~10???âÍ∑† ?òÏùµ: [0.32, -1.22, -0.99, -1.28, -1.88, -0.34, -0.95, -0.29, -0.21, 0.76]
     OBI <= -0.5 (Îß§ÎèÑ) 1~10???âÍ∑† ?òÏùµ: [-0.35, -1.12, -1.54, -1.83, -2.46, -3.32, -3.32, -3.88, -4.16, -5.3]
    
    [TRANSLATOR_ECLIPSE_CHARCOAL]
     OBI >= 0.5 (Îß§Ïàò) 1~10???âÍ∑† ?òÏùµ: [0.23, -0.3, -0.01, -0.44, 0.26, 0.83, 1.46, 2.43, 2.96, 3.37]
     OBI <= -0.5 (Îß§ÎèÑ) 1~10???âÍ∑† ?òÏùµ: [-0.33, -0.9, -0.75, -1.48, -2.25, -3.02, -3.21, -4.08, -4.89, -5.7]
    
    [TRANSLATOR_VOID_BLUE]
     OBI >= 0.5 (Îß§Ïàò) 1~10???âÍ∑† ?òÏùµ: [1.09, 1.45, 1.9, 2.13, 2.14, 2.65, 3.38, 5.11, 5.5, 5.35]
     OBI <= -0.5 (Îß§ÎèÑ) 1~10???âÍ∑† ?òÏùµ: [-0.19, -2.2, -3.29, -3.99, -4.81, -5.49, -5.18, -6.86, -7.85, -8.39]
...
# Visualization: 3√ó2 grid (one row per leader, buy vs. sell)
...
```
    ...
    === OBI ?ÑÍ≥ÑÍ∞?¬±0.5) ?åÌåå ??N???ÑÏ†Å Í∞ÄÍ≤?Î≥Ä???òÏùµ) ?âÍ∑† (Day 2) ===
    
    [Leader: SLEEP_POD_POLYESTER -> Follower: SLEEP_POD_NYLON]
     OBI >= 0.5 (Îß§Ïàò) 1~10???âÍ∑† ?òÏùµ: [2.9, 3.21, 3.24, 5.26, 3.67, 4.25, 3.72, 3.47, 3.33, 4.26]
     OBI <= -0.5 (Îß§ÎèÑ) 1~10???âÍ∑† ?òÏùµ: [-3.67, -3.41, -4.87, -3.98, -4.12, -5.36, -5.99, -6.11, -8.16, -9.38]
    
    [Leader: SLEEP_POD_NYLON -> Follower: SLEEP_POD_POLYESTER]
     OBI >= 0.5 (Îß§Ïàò) 1~10???âÍ∑† ?òÏùµ: [1.33, 1.39, 0.7, 0.12, 0.31, 1.24, 0.86, 0.69, 0.01, 0.52]
     OBI <= -0.5 (Îß§ÎèÑ) 1~10???âÍ∑† ?òÏùµ: [-1.44, -1.29, -1.05, -0.38, -0.8, -0.19, -0.61, -0.48, 0.11, 0.24]
...
# Visualization: 2 pairs √ó 2 directions = 4 charts total
